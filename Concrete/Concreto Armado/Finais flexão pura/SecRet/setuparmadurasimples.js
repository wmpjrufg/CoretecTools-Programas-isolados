  ...
  for (let j = 0; j < camadaBolinha.length ; j++) {
    for (let i = 0; i < camadaBolinha[j]; i++) {
        if(camadaBolinha[j] == 1)
        {
          colorMode(HSB);
          fill(131, 29, 28);
          circle(shearSteelInternox1 + (offset/2), shearSteelInternoy1 + hEstriboInterno - (offset/2) - j * (av+offset), phi);
        }else{
          colorMode(HSB);
          fill(131, 29, 28);
          circle(shearSteelInternox1 + (offset/2) + (i *  spacing[j]), shearSteelInternoy1 + hEstriboInterno - (offset/2) - j * (av+offset), phi);
