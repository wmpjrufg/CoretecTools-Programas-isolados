...
// ---------- Chamada de funções
//
include_once('LAJEFLEXAOPURACONCRETOARMADO02_v00_erros.php');
include_once('LAJEFLEXAOPURACONCRETOARMADO02_v00_avisos.php');
include_once('LAJEFLEXAOPURACONCRETOARMADO02_v00_detalhamento.php');
include_once('LAJEFLEXAOPURACONCRETOARMADO02_v00_armadura.php');
...
echo "-----------------------------------------------\n";
echo "DETERMINAÇÃO DA ARMADURA NA DIREÇÃO Lx:\n";
echo "-----------------------------------------------\n";
$tipomomento="Positivo";

LAJEFLEXAOPURACONCRETOARMADO02_v00_armadura($tipomomento,$dx,$bw,$fcd,$fyd,$Mdx,$lambda,$alfac,$epsiloncu,$epsilonsyield,$fctksup,$ac,$w,$duct);

$kmd  = $Mdx * 100 / ($bw * pow ($dx, 2)* ($fcd/10));
$kx   = (1 - sqrt (1-(2*$kmd)/$alfac))/$lambda;
$kz   = (1-0.5*$lambda*$kx);
$As_1=0.2*(($Mdx*100)/($kz*$dx*$fyd/10));
$As_2=0.9;

$mdmin  = 0.8 * $w * ($fctksup/10);
$kmdmin = $mdmin / ($bw * pow ($dx, 2)* ($fcd/10));
$kxmin  = (1 - sqrt (1-(2*$kmdmin)/$alfac))/$lambda;
$kzmin  = (1-0.5*$lambda*$kxmin);
$asmin = $mdmin/($kzmin*$dx*$fyd/10);
$ro = ($asmin/$ac)*100;
if ($ro < 0.15){
  $asmin = $ac * 0.15/100;
} else{
...
$As_sec=max($As_1,$As_2,$As_3);

echo "As_sec = $As_sec cm2/m\n";
echo "-----------------------------------------------\n\n";

LAJEFLEXAOPURACONCRETOARMADO02_v00_armadura($d_neg,$bw,$fcd,$fyd,$Mdneg_1,$lambda,$alfac,$epsiloncu,$epsilonsyield,$fctksup,$ac,$w,$duct);
LAJEFLEXAOPURACONCRETOARMADO02_v00_armadura($d_neg,$bw,$fcd,$fyd,$Mdneg_2,$lambda,$alfac,$epsiloncu,$epsilonsyield,$fctksup,$ac,$w,$duct);
